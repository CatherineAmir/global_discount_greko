...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_eg_eta_prepare_eta_invoice(self, invoice):
        ...
        eta_invoice.update({
            ...
            'netAmount': self._l10n_eg_edi_round(totals['total_price_subtotal_before_discount']-totals['discount_total']),
    ...
    def _check_move_configuration(self, invoice):
        ...
        if not all(aml.product_uom_id.l10n_eg_unit_code_id.code for aml in invoice.invoice_line_ids.filtered(lambda x: not (x.display_type or 'discount' in x.name.lower()))):
            print("invoice.invoice_line_ids.filtered(lambda x: not (x.display_type or x.name=='Discount')",invoice.invoice_line_ids.filtered(lambda x: not (x.display_type or 'discount' in x.name.lower())))
            errors.append(_("Please make sure the invoice lines UoM codes are all set up correctly"))
        if not all(tax.l10n_eg_eta_code for tax in invoice.invoice_line_ids.filtered((lambda x: not (x.display_type or 'discount' in x.name.lower()))).tax_ids):
            errors.append(_("Please make sure the invoice lines taxes all have the correct ETA tax code"))
        if not all(aml.product_id.l10n_eg_eta_code or aml.product_id.barcode for aml in invoice.invoice_line_ids.filtered(lambda x: not (x.display_type or  'discount' in x.name.lower()))):
